...
function stoneImpact(x,y,n=6){debris(x,y,n);dust(x,y,Math.max(2,Math.ceil(n/2)),n>=9);sparks(x,y,Math.max(1,Math.floor(n/3)))}
...
function impactJuice(t,a,d){
 ...
 if(a){a.recoil=.11;const dx=t.x-a.x,dy=t.y-a.y,len=Math.hypot(dx,dy)||1;const force=a.dragon?8:(a.type==='knight'||a.type==='enemyKnight')?5:(a.type==='catapult'||a.type==='enemyCatapult')?9:2.4;if(!t.anchored&&t.type!=='tower'&&t.type!=='enemyTower'&&t.type!=='wall'){t.x+=dx/len*force;t.y+=dy/len*force}}
...
function drawJuiceOverlays(){
 ...
 for(const o of [...units,...enemies,...towers,...walls])if(o.flash>0){ctx.save();ctx.globalAlpha=clamp(o.flash/.07,0,1)*.8;ctx.fillStyle='#fff';ctx.fillRect(o.x-10,o.y-17,20,30);ctx.restore()}
}
function setDevOpen(open){devOpen=open;$('#devPanel').classList.toggle('hidden',!open)}
function updateDevPanel(){if(!devOpen)return;$('#devFps').textContent=Math.round(devFps);$('#devEnemies').textContent=enemies.length;$('#devUnits').textContent=units.length;$('#devParticles').textContent=parts.length;$('#devDeaths').textContent=deaths.length;$('#devPause').textContent=devPaused?'RESUME':'PAUSE';$('#devCoins').classList.toggle('active',devInfiniteMana);$('#devHitStop').classList.toggle('active',devHitStop);$('#devPlayerNation').classList.toggle('active',devPlayerNation);$('#devEnemyNation').classList.toggle('active',devEnemyNation);$('#devPlayerNation').textContent=`PLAYER NATION: ${devPlayerNation?'ON':'OFF'}`;$('#devEnemyNation').textContent=`ENEMY NATION: ${devEnemyNation?'ON':'OFF'}`}
function spawnDev(type){if(!started||ended)return;if(type==='hero'){const before=deployments;deployments=99;deploy('warrior',W/2,H*.72);deployments=Math.max(before,deployments);return}if(type==='enemyDragon'){const e0=elapsed;elapsed=Math.max(elapsed,331);const old=Math.random;Math.random=()=>0;spawn('enemyDragon');Math.random=old;elapsed=e0;return}const map={raider:[18,5,25,20,.82,1],berserker:[34,10,31,20,.62,3],enemyKnight:[32,8.5,46,24,.78,5],enemyCatapult:[17,15,10,185,.20,7]};const b=map[type]||map.raider;enemies.push({id:nextId++,type,x:W/2+rand(-80,80),y:40,hp:b[0],maxHp:b[0],damage:b[1],speed:b[2],range:b[3],rate:b[4],cool:0,reward:b[5],lastHitBy:null,splash:type==='enemyCatapult'?52:0,anim:0})}
...
window.addEventListener('DOMContentLoaded',()=>{
 $('#devClose').onclick=()=>setDevOpen(false);$('#devPause').onclick=()=>{devPaused=!devPaused;updateDevPanel()};$('#devCoins').onclick=()=>{devInfiniteMana=!devInfiniteMana;if(devInfiniteMana)mana=999;updateDevPanel()};$('#devHitStop').onclick=()=>{devHitStop=!devHitStop;updateDevPanel()};$('#devPlayerNation').onclick=()=>{devPlayerNation=!devPlayerNation;toast(`PLAYER NATION ADVANTAGE ${devPlayerNation?'ON':'OFF'}`);updateDevPanel()};$('#devEnemyNation').onclick=()=>{devEnemyNation=!devEnemyNation;toast(`ENEMY NATION ADVANTAGE ${devEnemyNation?'ON':'OFF'}`);updateDevPanel()};$('#devRaider').onclick=()=>spawnDev('raider');$('#devBerserker').onclick=()=>spawnDev('berserker');$('#devKnight').onclick=()=>spawnDev('enemyKnight');$('#devCatapult').onclick=()=>spawnDev('enemyCatapult');$('#devDragon').onclick=()=>spawnDev('enemyDragon');$('#devHero').onclick=()=>spawnDev('hero');$('#devKill').onclick=()=>{for(const e of enemies){e.hp=0;spawnDeath(e,true)}};$('#devKeep').onclick=()=>{keep=Math.max(0,keep-20);shake(8);debris(W/2,H-35,10);react('wince',1);updateUI()};
});
